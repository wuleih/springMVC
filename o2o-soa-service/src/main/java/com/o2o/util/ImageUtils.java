...
public class ImageUtils {
	...
	public final static void cut(String srcImageFile, String result, int x,
		...
		try {
			...
			if (srcWidth > 0 && srcHeight > 0) {
				...
				g.dispose();
				// 输出为文件
				String formatName  = result.substring(result.lastIndexOf(".")+1).toUpperCase();
	...
	public static void fill(String srcImageFile, String destImageFile,int width, int height,int imageWidth,int imageHeight,int imageX, int imageY){
		try {
			// 读取源图像
			BufferedImage bi = ImageIO.read(new File(srcImageFile));
						
			BufferedImage totalImage = new BufferedImage(width + imageWidth * 2, height + imageHeight * 2, BufferedImage.TYPE_INT_RGB);				
			Graphics2D g = totalImage.createGraphics();
			g.setColor(new Color(Integer.parseInt("CCCCCC", 16)));
			g.fillRect(0, 0, width + imageWidth * 2, height + imageHeight * 2);
			
			g.drawImage(bi, imageWidth + imageX, imageHeight
					+ imageY, imageWidth, imageHeight,
					Color.white, null);
			g.dispose();
			
			ImageIO.write((BufferedImage) totalImage, "JPEG", new File(destImageFile));
	...
	public final static boolean isImage(String imgFile){
		return isImage(new File(imgFile));		
	}
	
	public final static boolean isImage(File imgFile){
		BufferedImage bi;
		try {
	...
	public final static File getFileByKey(HttpServletRequest request, String key){
		String path = key;
		return new File(request.getSession().getServletContext().getRealPath(path));//实际路径
	}
}